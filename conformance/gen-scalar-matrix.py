...
def float_to_int_cell(width):
    ...
    body = [
        ...
        "println(cast(int, back == y))",  # 1 if the round-trip is exact
    ]
    return body, [1]


def shift_cell(op, width, sign):
    # Shift by a count >= the operand width. Spec (claude-notes Operators, Go):
...
def all_cells():
    ...
    for width in FLOAT_WIDTHS:
        body, exp = float_to_int_cell(width)
        yield "float-to-int", width, "unsigned", body, exp
    for op in SHIFTS:
        for width in WIDTHS:
            for sign in SIGNS:
                body, exp = shift_cell(op, width, sign)
                yield op, width, sign, body, exp
