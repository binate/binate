...
`iface.assert`, §14.10 type switch). Cells are GENERATED by
`conformance/gen-type-assert-matrix.py` (do not edit by hand); `conformance/run.sh`
auto-discovers every `.bn` here.
