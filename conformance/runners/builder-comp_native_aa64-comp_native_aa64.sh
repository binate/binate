...
runner_exec() {
    ...
    compile_out=$("$BNC_NATIVE" -I "$compile_root:$BINATE_DIR/ifaces/core:$BINATE_DIR/ifaces/stdlib" -L "$compile_root:$BINATE_DIR/impls/core/common:$BINATE_DIR/impls/core/libc:$BINATE_DIR/impls/stdlib/common" --runtime "$BINATE_DIR/runtime/binate_runtime.c" --build-dir "$bdir" -backend native $BINATE_FLAGS -o "$tmpbin" "$bn" 2>&1) || true
    if [ -x "$tmpbin" ]; then
