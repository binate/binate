...
#       needs a *target-arch* libm.so.6: the host's when running natively, else one
#       copied out of the Docker image.  Gated on CAN_RUN (the same condition under
#       which a target libm.so.6 is obtainable); SKIP otherwise. -----
if [ "$CAN_RUN" = 1 ]; then
    LIBM_DIR="$TMP/libm"
    mkdir -p "$LIBM_DIR"
    _libm_ok=0
    if [ "$RUN_KIND" = native ]; then
        for _c in /lib/x86_64-linux-gnu/libm.so.6 /usr/lib/x86_64-linux-gnu/libm.so.6 /lib64/libm.so.6; do
            [ -e "$_c" ] && { ln -sf "$_c" "$LIBM_DIR/libm.so"; _libm_ok=1; break; }
        done
    else
        _cid="$(docker create --platform linux/amd64 debian:stable-slim)"
        if docker cp "$_cid:/lib/x86_64-linux-gnu/libm.so.6" "$LIBM_DIR/libm.so.6" > /dev/null 2>&1; then
    ...
    if [ "$_libm_ok" = 1 ]; then
        if ! "$BNLD" -target linux-x64 -dynamic -L "$LIBM_DIR" -lm -o "$TMP/exit42lm" "$TMP/exit42.o" \
        ...
        echo "SKIP: multi-lib -l — could not obtain a target libm.so.6"
    fi
else
