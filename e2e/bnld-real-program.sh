...
if command -v clang > /dev/null 2>&1; then
    ...
    # A minimal aarch64 shim: it only needs to DEFINE _start and the libc symbols
    # so the graph links (it is not run, so the stubs need not work).  rt.MemZero is
    # NOT supplied here: aarch64's rt.MemZero is a #[build]-gated package `.s` that
    # bnc assembles into its own `-c` object set, so the `bnc -c` output already
    # defines the symbol — supplying it here too would be a duplicate.
