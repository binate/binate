...
check "interp/with-args" "$WITH_ARGS" \
    "$("$BNI_BIN" -I "$CK_I" -L "$CK_L" -main-file "$TMP/fmt_os_args.bn" -- alpha beta gamma 2>&1)"
check "interp/no-args"   "$NO_ARGS" \
    "$("$BNI_BIN" -I "$CK_I" -L "$CK_L" -main-file "$TMP/fmt_os_args.bn" 2>&1)"
check "interp/empty-arg" "$EMPTY_ARG" \
    "$("$BNI_BIN" -I "$CK_I" -L "$CK_L" -main-file "$TMP/fmt_os_args.bn" -- alpha '' gamma 2>&1)"
