...
# NOT the pinned BUILDER directly: os depends on a compiler fix that postdates
# BUILDER_VERSION, so the prebuilt BUILDER cannot compile os yet.  Once BUILDER
# is bumped past that fix this can revert to the simpler `$BUILDER … cmd/bnc --
# …` form other e2e scripts use (shared with stat-values.sh; see todo: "drop the
# gen1 build in the os e2e scripts after the next BUILDER bump").
GEN1="$TMP/gen1"
