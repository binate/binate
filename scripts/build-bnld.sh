#!/bin/sh
# Build the self-hosted linker (bnld).
#
# Convenience wrapper around the BUILDER → gen1 bnc → cmd/bnld build
...
#
# Usage:
#   ./scripts/build-bnld.sh -o <path>         # release (-O2)
#   ./scripts/build-bnld.sh -o <path> --debug # -O0 -g (slower, debuggable)
#   ./scripts/build-bnld.sh -h                # help
#
# --target <key> cross-compiles Stage 2 for a bnc --target key (e.g.
# aarch64-linux, x86_64-darwin); omitted builds for the host arch.
#
# After building:
#   <path> -o <out> [-e entry] [-target linux-x64] <obj.o>...  link to executable
...
echo "Building bnld: $MODE_DESC → $OUT"
...
echo "  Stage 2: gen1 → bnld ..."
# Cross-compile: when --target names a non-host target, gen1 (a host binary,
# built above) cross-EMITS the final bnld for that target.  The one key drives
...
echo
echo "Try:"
echo "  $OUT -o /tmp/prog foo.o        # link objects to an executable"
