...
#         -I "$BINATE_DIR:$BUILDER_LIB" \
#         -L "$BINATE_DIR:$BUILDER_LIB" ...
#
# Modes:
...
# Historic builder-link callers invoke the resolved BUILDER as
#   "$BUILDER" -root "$BINATE_DIR" "$BINATE_DIR/cmd/bnc" -- <bnc-args>
# because under bootstrap-* mode BUILDER is the bootstrap interpreter
# and that prefix is bootstrap's flag for "load cmd/bnc from <dir>".
...
case "$BUILDER_VERSION" in
    ...
    bnc-*)
        ...
        if [ "$mode" = lib ]; then
            ...
            # Wrapper-script indirection so callers keep their
            # historic bootstrap-shape invocation
            # (`$BUILDER -root <dir> <src> -- <args>`) without
            # caring whether the resolved BUILDER is bootstrap
            # (which expects that shape literally) or a bnc
            # binary (which does not).  The wrapper strips a
            # leading `-root <dir> <src> --` prefix when it
            # looks like the bootstrap shape, then exec's the
            # bundled bnc / bni / etc.  Direct callers (no
            ...
            if [ ! -x "$WRAPPER" ]; then
                ...
                cat > "$WRAPPER" <<EOF
#!/bin/sh
# Generated by scripts/fetch-builder.sh.  See the wrapper
# comment in that script for the prefix-stripping rationale.
if [ "\$1" = "-root" ] && [ "\$#" -ge 4 ] && [ "\$4" = "--" ]; then
    shift 4
fi
exec "$REAL_BIN" "\$@"
EOF
